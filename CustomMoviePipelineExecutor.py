...
class CustomMoviePipelineExecutor(unreal.MoviePipelinePythonHostExecutor):
    ...
    def _post_init(self):
        ...
        self.on_executor_errored_delegate.add_function_unique(self, "on_executor_errored")
        ...
        # Replace this URL with your REST API endpoint
        # newIndex = self.send_http_request("https://google.com", "GET", "", unreal.Map(str, str)) # This is an example of how to make a REST request.
        
        rest_api_url = "http://100.87.37.19:3000/api/test/unrealEngineTest"
        newIndex = self.send_http_request(rest_api_url, "GET", "", unreal.Map(str, str))
    ...
    def execute_delayed(self, inPipelineQueue):
        ...
        socketConnected = self.connect_socket("http://100.87.37.19", 3000)
        ...
        try:
            ...
            unreal.log_error("Missing '-LevelSequence=/Game/Example/MySequence.MySequence' argument")
            self.on_executor_errored(self, self.activeMoviePipeline, True, "Missing '-LevelSequence=/Game/Example/MySequence.MySequence' argument")
            return
    ...
    def on_begin_frame(self):
        ...
        super(CustomMoviePipelineExecutor, self).on_begin_frame()        
    ...
    @unreal.ufunction(ret=None, params=[unreal.MoviePipelineExecutorBase, unreal.MoviePipeline, bool, unreal.Text])
    def on_executor_errored(self, inMoviePipelineExecutor, inMoviePipeline, bFatal, error_reason):
        unreal.log_error(f"Error during execution! Reason: {error_reason}!")
        unreal.log(f"FATAL: {bFatal}")
        self.activeMoviePipeline = None
        self.on_executor_finished_impl()
    ...
    def on_http_response_recieved(self, inRequestIndex, inResponseCode, inMessage):
        print(f"Got response message:  {inMessage}")
        print(f"Got response code: {inResponseCode}")
        print(f"request index:  {inRequestIndex}")
